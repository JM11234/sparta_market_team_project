...
class Product(models.Model):
    ...
    price = models.DecimalField(max_digits=10, decimal_places=2)  ## 상품가격, 최대 10자리 숫자와 소수점 2자리까지 허용
    is_available = models.BooleanField(default=True)   ## 판매 가능 여부
    created_at = models.DateTimeField(auto_now_add=True)
    updated_at = models.DateTimeField(auto_now=True)
    main_image = models.ImageField(upload_to='images/', blank=True, null=True) ## 상품 메인이미지
